...
const get_owner = async () => {
  var owner = await reward.methods.contract_owner().call();
...
const get_contractBalance = async () => {
  ...
  console.log("5.当前合约余额:",Big(balance).div(
    10 ** 18
  ).toFixed(0));
};
...
const get_reward_amount = async () => {
  let result = await reward.methods
    ...
    console.log("当前提现金额:",Big(result).div(
      10 ** 18
    ).toFixed(0));
};

const get_next_reward_time = async (imei:string) => {
  let result = await reward.methods
...
const set_reward_amount = async (amount:number) => {
  ...
  web3.eth
    .sendSignedTransaction(signed.rawTransaction)
    .on("receipt", (receipt) => {
      console.log("set reward amount receipt", receipt);
...
const transfer_erc20 = async (token_contract_address:string, watch_id:string,report_hash:string,receive_address:string) => {
  console.log("call set amount function......");
  ...
  const tx: any = {
    ...
    gas: 5000000,
    data: reward.methods
      .transfer_erc20(token_contract_address,watch_id,report_hash,receive_address)
      .encodeABI(),
  };
...
const set_amount = Big(amount).times(10 ** 18).toFixed(0);
